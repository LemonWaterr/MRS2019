...
def braitenberg(front, front_left, front_right, left, right):
  ...
  u = np.tanh(front-0.1)
  w = max(min((fl-fr) * np.pi/4.0, np.pi/4.0), -np.pi/4.0)
...
class Particle(object):
  ...
  def __init__(self):
    ...
    self._pose[X] = random.uniform(-2.0,2.0)
    self._pose[Y] = random.uniform(-2.0,2.0)
    self._pose[YAW] = random.uniform(0, np.pi * 2)
  ...
  def is_valid(self):
    ...
    if x>2.0 or x<-2.0 or y>2.0 or y<-2.0:
      return False
    elif math.sqrt((CYLINDER_POSITION[0]-x)**2 + (CYLINDER_POSITION[1]-y)**2) < CYLINDER_RADIUS:
      return False
    else:
      return True
  ...
  def move(self, delta_pose):
    ...
    if random.random() < 0.1:  #kidnapped robot by 1%
      self._pose[0] = random.uniform(-2.0,2.0)
      self._pose[1] = random.uniform(-2.0,2.0)
      while not self.is_valid():
        self._pose[0] = random.uniform(-2.0,2.0)
        self._pose[1] = random.uniform(-2.0,2.0)
    else:
      u = 0.0 if delta_pose[0] < 0.000001 else np.random.normal(delta_pose[0], abs(delta_pose[0]*0.1))
      w = 0.0 if delta_pose[2] < 0.000001 else np.random.normal(delta_pose[2], abs(delta_pose[2]*0.1))
  ...
  def compute_weight(self, front, front_left, front_right, left, right):
    ...
    distances = [front, front_left, front_right, left, right]
    distances_computed = [self.ray_trace(angle) for angle in angles]
    distances_computed_num = [infToNum(x) for x in distances_computed]
    distributions = [scipy.stats.norm(distance, sigma) for distance in distances_computed_num]
    
    weight_prod = 1.0
    weight_count = 0
    
    for i in range(0,5):
      if distances[i] == 'inf' or distances_computed[i] == 'inf':
        continue
      else:
        weight_prod *= distributions[i].pdf(distances[i])
        weight_count += 1
    
    if weight_count != 0:
      #print(weight_prod)
      self._weight = weight_prod
    else:
      self._weight = 0.0
